**************************
collective.atomsyndication
**************************

Life, the Universe, and Everything
----------------------------------

An `Atom`_ feed generator for Plone sites.

Atom is an XML-based document format that describes lists of related
information known as "feeds". Feeds are composed of a number of items, known
as "entries", each with an extensible set of attached metadata. For example,
each entry has a title.

The primary use case that Atom addresses is the syndication of Web content
such as weblogs and news headlines to Web sites as well as directly to user
agents.

Atom feeds have multiple `advantages`_ over RSS feeds.

Don't Panic
-----------

TBD.

Plone 4.3
^^^^^^^^^

Please note that starting Plone 4.3 `Atom is among the standard supported
syndication formats`_, so this package will not be needed.

Mostly Harmless
---------------

.. image:: https://secure.travis-ci.org/collective/collective.atomsyndication.png
    :target: http://travis-ci.org/collective/collective.atomsyndication

Have an idea? Found a bug? Let us know by `opening a support ticket`_.

.. _`Atom`: https://www.ietf.org/rfc/rfc4287.txt
.. _`advantages`: http://www.intertwingly.net/wiki/pie/Rss20AndAtom10Compared
.. _`Atom is among the standard supported syndication formats`: https://dev.plone.org/ticket/12908
.. _`opening a support ticket`: https://github.com/collective/collective.atomsyndication/issues
